...
class PaddingStrategy:
    ...
    def apply(
        cls,
        context: PaddingContext,
        shape: Tuple[int, ...],
        dtype: np.dtype = SCALAR_NP_TYPE,
    ) -> Tuple[int, ...]:
...
class LossBufferSpec(BufferSpec):
    ...
    def __post_init__(self):
        ...
        self.logical = LogicalShape(batch=self.real_shape[1], features=(self.real_shape[0],))
...
class InputBufferSpec(BufferSpec):
    ...
    def __post_init__(self):
        self.logical = LogicalShape(batch=self.real_shape[0], features=self.real_shape[1:])
        self.padded = PaddedShape(self.padded_shape)
...
class TargetsBufferSpec(BufferSpec):
    ...
    def __post_init__(self):
        self.logical = LogicalShape(batch=self.real_shape[0], features=())
        self.padded = PaddedShape(self.padded_shape)
...
ACCESS_WRITE_MODES: Set[AccessMode] = {
    AccessMode.EXCLUSIVE_WRITE_OVER,
    AccessMode.EXCLUSIVE_ZERO,
    AccessMode.EXCLUSIVE_UPDATE,
}
...
class WorkManager:
    ...
    def __init__(self, context: cl.Context):
        ...
        self.buffer_tracker = defaultdict(
            lambda: {
                "version": 0,
                "alloc_node": None,
                "free_node": None,
    ...
    def create_sync_node(self, sync_type: SyncType, dependencies: List[int] = []) -> int:
        ...
        node = self.ExecutionNode(
            ...
            {},
            set(dependencies),
            {"sync_type": sync_type, "user_event": user_event},
        )
        self.execution_graph.add_node(node.uid, node=node)
        if user_event:
    ...
    def commit_workload(self) -> None:
        ...
        for uid in ordered_nodes:
            ...
            for res, modes in node.access_map.items():
                if res in self.logical_resources and (
                    AccessMode.SHARED_READ in modes or AccessMode.EXCLUSIVE_UPDATE in modes
                ):
    ...
    PARAM_TYPES: Dict[str, Dict[str, Union[Callable, Tuple[float, float]]]] = {
        ...
        "exit_weight": {
            "post_pad_fn": lambda padded, sw: padded.reshape(
                padded.shape[0], sw, padded.shape[1] // sw, padded.shape[2]
            )
        },
        "temperature": {
...
class KernelExecutionRequest:
    ...
    def bind_argument(
        self,
        arg_index: int,
        arg: Union[cl.Buffer, np.ndarray, np.int32, SCALAR_NP_TYPE],
    ) -> None:
...
class KernelWrapper:
    ...
    def __init__(
        ...
        manager: WorkManager,
        global_step: int,
        compute_queue: cl.CommandQueue,
        data_manager: DataBufferManager,
        parameter_manager: ParameterBufferManager,
    ):
        self.program = program
        ...
        self.padded_input_dim = self.data_manager.create_data_buffer(
            "input_batch",
            (BATCH_SIZE, INPUT_DIM),
            SCALAR_NP_TYPE,
            BufferPurpose.INPUT_DATA,
        ).spec.padded.values[1]
        self.padded_hidden_dim = self.data_manager.create_data_buffer(
    ...
    def forward_pass(
        ...
        global_sizes: Tuple[int, ...],
        local_sizes: Tuple[int, ...],
        input_buf: DataBuffer,
        weights_buf: ParameterBuffer,
        biases_buf: ParameterBuffer,
        hidden_buf: DataBuffer,
    ) -> int:
    ...
    def compute_exit_probabilities(
        ...
        local_sizes: Tuple[int, ...],
        hidden_buf: DataBuffer,
        exit_weights_buf: ParameterBuffer,
        exit_biases_buf: ParameterBuffer,
    ...
    def compute_gradients(
        ...
        exit_probs_buf: DataBuffer,
        exit_weights_buf: ParameterBuffer,
        grad_weights_buf: ParameterBuffer,
        grad_biases_buf: ParameterBuffer,
        grad_exit_weights_buf: ParameterBuffer,
        grad_exit_biases_buf: ParameterBuffer,
        targets_buf: DataBuffer,
    ) -> int:
    ...
    def adam_update(
        ...
        global_sizes: Tuple[int, ...],
        local_sizes: Tuple[int, ...],
        grad_buf: ParameterBuffer,
        param_buf: ParameterBuffer,
        m1_buf: ParameterBuffer,
        m2_buf: ParameterBuffer,
        grad_name: str,
        param_name: str,
        m1_name: str,
        m2_name: str,
    ...
    def clamp_temperatures(
        self,
        global_sizes: Tuple[int, ...],
        local_sizes: Tuple[int, ...],
        temps_buf: ParameterBuffer,
    ) -> int:
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(num_batches):
        ...
        grad_node = kernel_wrapper.compute_gradients(
            ...
            exit_probs_buf,
            pm.buffers["exit_weights"],
            pm.buffers["grad_weights"],
            pm.buffers["grad_biases"],
            pm.buffers["grad_exit_weights"],
            pm.buffers["grad_exit_biases"],
            targets_buf,
        )
        ...
        sync_uid = manager.create_sync_node(
            SyncType.HOST_SIGNAL,
            dependencies=[losses_transfer, exit_probs_transfer, temps_transfer],
        )
        manager.commit_workload()
        ...
        valid_exit_probs = exit_probs_view.valid_slice.transpose(1, 0, 2).astype(
            np.float32
        )  # Shape (actual_batch, NUM_EXITS, OUTPUT_CLASSES)
