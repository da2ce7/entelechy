...
#pragma unroll 4
    ...
    __local SCALAR_TYPE loss_reduce[WG_SIZE];
...
#pragma unroll 4
    ...
    // Phase 3: Final probabilities and loss calculation
    __local int class_loc;

    // 1. Single-pass parallel search
    SCALAR_TYPE private_loss = 0;
    for (uint c = lid; c < output_classes; c += lsize) {
        const SCALAR_TYPE prob      = exit_probs[exit_offset + c] / sum_total;
        exit_probs[exit_offset + c] = prob;

        if ((int)c == true_class) {
            private_loss = -native_log(fmax(prob, SAFE_LOG_MIN));
            class_loc    = lid; // Track location
        }
    }

    // 2. Loss gathering with broadcast
    barrier(CLK_LOCAL_MEM_FENCE);
    if (lid == class_loc) {
        loss_reduce[0] = private_loss;
    }

    barrier(CLK_LOCAL_MEM_FENCE);
    if (lid == 0) {
        losses[exit_idx * padded_batch_size + batch_idx] = loss_reduce[0];
    }
