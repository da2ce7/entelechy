...
// --- Phase 5-7: Exit Layer Forward Pass & Loss ---
...
__kernel void compute_logits_chunk(
    ...
    int exit_chunk_id,                                       // [IN scalar: >= 0, Logical EXIT chunk index]
    int exit_param_offset,                                   // [IN scalar: >= 0, Starting EXIT index for this chunk]
    int num_exits_in_chunk,                                  // [IN scalar: > 0, Number of exits in this chunk]
...
__kernel void compute_probs_loss_cce_chunk(
    __global const SCALAR_TYPE *__restrict full_logits_buf,    // [IN]  Shape: (total_exits, total_batch_size, total_output_classes)
    __global const SCALAR_TYPE *__restrict softmax_params_buf, // [IN]  Shape: (total_exits, total_batch_size, 2)
    __global const SCALAR_TYPE *__restrict temps_buf,          // [IN]  Shape: (total_exits)
    ...
    int exit_chunk_id,                                         // [IN scalar: >= 0]
    int exit_param_offset,                                     // [IN scalar: >= 0]
    int num_exits_in_chunk,                                    // [IN scalar: > 0]
...
__kernel void backprop_error_to_hidden_chunk(
    ...
    __global const void *__restrict targets_buf,              // [IN]  Shape: Generic, cast based on problem_type_flag
    __global const SCALAR_TYPE *__restrict exit_weights_buf,  // [IN]  Shape: (total_exits, hidden_dim, total_output_classes)
    __global SCALAR_TYPE *__restrict partial_grad_h_aos_out,  // [OUT] Shape: (num_class_chunks, total_exits, total_batch_size, hidden_dim)
    int problem_type_flag,                                    // [IN scalar: 0|1, CCE or BCE]
    int exit_chunk_id,                                        // [IN scalar: >= 0]
    int exit_param_offset,                                    // [IN scalar: >= 0]
    int num_exits_in_chunk,                                   // [IN scalar: > 0]
...
__kernel void clamp_temperatures(
    ...
    int         total_exits);                           // [IN scalar: > 0, The number of temperatures to clamp]
