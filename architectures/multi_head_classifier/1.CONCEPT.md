## **Architectural Concept: A Unified, Memory-Aware Streaming Classification Engine (Revision 5)**
...
graph TD
    ...
    subgraph "Phase 0-3: Host Setup & Global Params"
        ...
        HL_Placement --> HL_Indirection["6a. Construct Indirection Lists"]:::host_logic
    ...
    subgraph "PHASE 2: LEARN"
        ...
        subgraph "Phase 11: Partial Gradient Clipping"
            style "Phase 11: Partial Gradient Clipping" loop_box
            K11_Clip["<b>(11) clip_partial_gradients</b><br/>(Generic Utility Kernel)"]:::utility_kernel
        end
        PARTIALS_Grad_ModW & PARTIALS_Grad_ModB & PARTIALS_Grad_Temps --> K11_Clip
        PARTIALS_Grad_H_AoS --> K11_Clip

        K11_Clip --> Clipped_PARTIALS_Grad_ModW["Clipped<br/>PARTIAL Grad_ModW"]:::clipped_data
        K11_Clip --> Clipped_PARTIALS_Grad_ModB["Clipped<br/>PARTIAL Grad_ModB"]:::clipped_data
        K11_Clip --> Clipped_PARTIALS_Grad_Temps["Clipped<br/>PARTIAL Grad_Temps"]:::clipped_data
        K11_Clip --> Clipped_PARTIALS_Grad_H_AoS["Clipped<br/>PARTIAL Grad_H (AoS)"]:::clipped_data
        K11_Clip --> Clipped_PARTIALS_Grad_SW_i["Clipped<br/>PARTIAL Grad_SW 'i'"]:::clipped_data
        K11_Clip --> Clipped_PARTIALS_Grad_SB_i["Clipped<br/>PARTIAL Grad_SB 'i'"]:::clipped_data

        subgraph "Phase 12-19: Gradient Processing & Reduction"
            subgraph "Phase 12-13: Grad_H Permutation"
                 K13["<b>(13) gather_and_permute_grad_h</b><br/>[Specialized Permutation]<br/><b>[Item Synchronization Point]</b>"]:::permute_kernel
                 Clipped_PARTIALS_Grad_H_AoS --> K13; K13 --> Permuted_Grad_H_SoA["Permuted Grad_H"]:::full_intermediate
                 K12["(12) transpose_chunk..."]:::kernel
            end

            subgraph "Phase 14: Recursive Reduction (Act Results)"
            ...
            subgraph "Phase 15: Recursive Reduction (Module Grads)"
                 K_Recursive_15["<b>(15) Recursive Reduction...</b>"]:::host_logic
            ...
            subgraph "Phase 16: Specialized Grad_H Reduction"
                K16["<b>(16) reduce_grad_h_over_modules</b>"]:::specialized_kernel
            ...
            subgraph "Phase 17-18: True Streaming Shared Layer Backprop"
                style "Phase 17-18: True Streaming Shared Layer Backprop" parallel_group
                PARTIALS_Grad_SW_i["PARTIAL Grad_SW 'i'"]:::partial_data
                PARTIALS_Grad_SB_i["PARTIAL Grad_SB 'i'"]:::partial_data

                Input_i[Input Chunk 'i']:::data & SampleMask --> K17["<b>(17) backprop_shared_weights...</b>"]:::kernel; K17 --> PARTIALS_Grad_SW_i
                hidden_i --> K17 & K18; Summed_Grad_H -- slice --> K17 & K18
                SampleMask --> K18["<b>(18) backprop_shared_biases...</b>"]:::kernel; K18 --> PARTIALS_Grad_SB_i

                PARTIALS_Grad_SW_i & PARTIALS_Grad_SB_i --> K11_Clip
            end

            subgraph "Phase 19: Recursive Reduction (Shared Grads)"
                 K_Recursive_19["<b>(19) Recursive Reduction...</b>"]:::host_logic
                 Offset_List & Clipped_PARTIALS_Grad_SW_i --> K_Recursive_19
                 Offset_List & Clipped_PARTIALS_Grad_SB_i --> K_Recursive_19
                 K_Recursive_19 --> Summed_Grad_SW[Summed Grad_SW]:::summed_data & Summed_Grad_SB[Summed Grad_SB]:::summed_data
            end
        end

        subgraph "Phase 20: Gradient Normalization"
            style "Phase 20: Gradient Normalization" loop_box
            K20["<b>(20) normalize_gradients</b><br/>(Generic Utility Kernel)"]:::utility_kernel
        end
        Summed_Grad_ModW & Summed_Grad_ModB & Summed_Grad_Temps --> K20
        Summed_Grad_SW & Summed_Grad_SB & Summed_Grad_H --> K20
        K20 --> FINAL_Grad_ModW[Final Grad_ModW]:::final_data & FINAL_Grad_ModB[Final Grad_ModB]:::final_data & FINAL_Grad_Temps[Final Grad_Temps]:::final_data
        K20 --> FINAL_Grad_SW[Final Grad_SW]:::final_data & FINAL_Grad_SB[Final Grad_SB]:::final_data & FINAL_Grad_H[Final Grad_H]:::final_data

        subgraph "Phase 21: Batch Synchronization Point"
            style "Phase 21: Batch Synchronization Point" sync_barrier
            K21_Barrier["<b>(21) Batch Synchronization Point</b>"]:::sync_barrier
        end
        FINAL_Grad_ModW & FINAL_Grad_ModB & FINAL_Grad_Temps --> K21_Barrier
        FINAL_Grad_SW & FINAL_Grad_SB & FINAL_Grad_H --> K21_Barrier

        subgraph "Phase 22: Asynchronous Result Retrieval"
            direction LR; K22["<b>(22) D2H Async Copy</b><br/>(Final Probs)"]:::data --> EV_Inference["<b>inference_event</b>"]:::sync_event
        end
        FINAL_Probs --> K22; EV_Inference --> Host_Act["Host Acts on<br/>Full Forward Result"]:::host_logic

        subgraph "Phase 23-24: Atomic Batch Update"
            direction LR; K21_Barrier --> K23_shared & K23_module & K23_temps
            K23_shared["(23) adam_update (Shared)"]:::kernel; FINAL_Grad_SW & FINAL_Grad_SB & P_Adam_Host --> K23_shared; K23_shared -- updates --> P_Shared
            K23_module["(23) adam_update (ClassifierModule)"]:::kernel; FINAL_Grad_ModW & FINAL_Grad_ModB & P_Adam_Host --> K23_module; K23_module -- updates --> P_ClassifierModule
            K23_temps["(23) adam_update (Temps)"]:::kernel; FINAL_Grad_Temps & P_Adam_Host --> K23_temps; K23_temps -- updates --> P_Temps
            K23_temps --> K24["<b>(24) clamp_temperatures</b>"]:::kernel; K24 --> EV_Final["<b>final_batch_event</b>"]:::sync_event
        end
    end
...
- **Batch Synchronization Point:** A barrier that resolves a data dependency _between multiple independent learning items_ that constitute a single logical batch. It ensures that the parallel-processed, fully-reduced, and normalized gradients for all items are ready before the final, collective state-update step is performed. Its purpose is to enforce the correctness of the parallel training paradigm (e.g., mini-batch gradient descent).
  - **Canonical Example:** Node **(21) Batch Synchronization Point**.
...
- **(11) `clip_partial_gradients`**: **[Utility Kernel]** Applies gradient clipping to each per-item partial gradient.
  - **Contract:** Invoked after all partial gradients for an item are generated. It calculates the L2 norm of the gradient vectors for a given item and, if the norm exceeds a host-provided threshold, scales the vector down. This step is critical for training stability, especially with low-precision formats. Consumes `PARTIAL_*` gradient buffers and produces `Clipped_PARTIAL_*` buffers.
...
- **(14), (15), (19) `aggregate_*` kernels**: A suite of stateless reduction kernels serving as the **Recursive, Tiered Aggregation Engine**.
  - **Contract:** This engine is composed by the host to aggregate scattered partial results. Instead of requiring a contiguous input buffer, these kernels accept a pointer to a generic memory pool (`partial_collection`) and an indirection table (`partial_offset_list`). This contract avoids expensive intermediate memory copies, making the reduction highly efficient. They sum Partial Probabilities (`Node 14`), **Clipped** Partial Module Gradients (`Node 15`), and **Clipped** Partial Shared Gradients (`Node 19`). The output of a gradient aggregation is a **`Summed_*`** gradient buffer.
...
- **(17) `backprop_shared_weights_chunk`**: Computes partial gradients for shared weights.
  - **Contract:** Consumes the `Summed_Grad_H` and a **chunk** of recomputed `hidden_i`. Adheres to the Placement Contract.
  - **Lifecycle Note:** The `PARTIAL_Grad_SW_i` buffer produced by this kernel **must** be processed by Node (11) before being aggregated by Node (19).
- **(18) `backprop_shared_biases_chunk`**: Computes partial gradients for shared biases.
  - **Contract:** Consumes the `Summed_Grad_H` and a **chunk** of recomputed `hidden_i`. Adheres to the Placement Contract.
  - **Lifecycle Note:** The `PARTIAL_Grad_SB_i` buffer produced by this kernel **must** be processed by Node (11) before being aggregated by Node (19).
- **(20) `normalize_gradients`**: **[Utility Kernel]** Generic, element-wise scaling kernel.
  - **Contract:** Performs an element-wise division of an input buffer by a scalar value (`N`, the number of items in the batch). It consumes all `Summed_*` parameter gradient buffers and produces their corresponding **`Final_*`**, normalized gradient buffers, which are essential for numerically stable mini-batch training.

---

#### **Final Update & Retrieval Kernels**

- **(22) `D2H Async Copy`**: A non-blocking Device-to-Host transfer of `Final Probs`, signaling completion via `inference_event`.
- **(23) `adam_update`**: Generic optimizer kernel, invoked once per complete parameter group **per training batch**, after the **Batch Synchronization Point**.
...
- **(24) `clamp_temperatures`**: Final utility kernel constraining temperature parameters.
...
- **Scenario: The Marathon (Massive `epochs`)**
  ...
  - **Insight:** Guarantees **long-term stability** by delegating the sensitive `beta**t` calculation to the host and passing the final values to the `(23) adam_update` kernel, avoiding on-device precision loss and ensuring the optimizer remains mathematically correct indefinitely.

- **Scenario: The Hydra (Massive `num_heads`)**

  - **Insight:** Validates the intelligence of the **Reduction Planner**. For a massive number of heads, the host renders an efficient `log_K(N)` reduction tree, not just for diagnostics like loss, but for the core accumulation of **clipped** `Grad_ModW` partials, proving the engine is the heart of the learning algorithm itself.
...
- **Scenario: The Rodeo (Extreme Instability Resilience)**
  ...
  - **Key Insight:** Proves the critical role of **`(11) clip_partial_gradients`** as a non-optional, foundational stability primitive. By enforcing a maximum norm on every per-item partial gradient _before_ any reduction takes place, the architecture guarantees that intermediate values in the reduction tree and the final summed gradients cannot overflow the limited dynamic range of low-precision formats. This ensures the engine remains numerically robust not just by policy, but **by design**, even under the most adverse training conditions.
...
- **Scenario: The Scientist's Repeater (Numerical Correctness Validation)**
  ...
  - **Key Insight:** Proves the correctness of the **`(20) normalize_gradients`** kernel. By demonstrating that the learning dynamics are independent of batch size, it validates that the system is computing the true _average_ gradient, not the sum, which is critical for predictable hyperparameter tuning and algorithmic stability.
...
- **Scenario: The Data Tsunami (Massive `batch_size`)**
  ...
  - **Insight:** This is the canonical test of the full parallel learning model. It validates that the host correctly processes a large training batch (`N` items) by:
    ...
    2.  Correctly inserting the **`(11) clip_partial_gradients`** step for each item's gradients.
    ...
    6.  Correctly inserting the **`(20) normalize_gradients`** step.
    7.  Enforcing the strict **`(21) Batch Synchronization Point`**, ensuring the `(23) adam_update` kernel only runs _after_ all preceding steps are complete.
...
- **Scenario: The Colossus (Holistic Stress Test)**
  - **Insight:** Tests the synergy of all scaling strategies under extreme, compound memory pressure. This proves the orchestrator's robustness by forcing it to compose a single, valid execution graph that correctly handles:
    1.  Per-item gradient clipping (`Node 11`).
    2.  The **Item Synchronization Point** (`Node 13`) for the `Grad_H` calculation.
    3.  The **Batch Synchronization Point** (`Node 21`) for the final update.
    4.  The **Recursive Reduction Engine** applied over multiple, independent data paths.
    5.  The **dual streaming models** for recomputing inputs on demand.
