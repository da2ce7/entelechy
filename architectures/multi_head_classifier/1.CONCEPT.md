...
graph TD
    ...
    subgraph "PHASE 2: LEARN"
        ...
        %% Note: The true streaming path for SW/SB grads would also need clipping before reduction.
        %% This diagram shows the logic for the main path.
        ...
        subgraph "Phase 12-19: Gradient Processing & Reduction"
            ...
            subgraph "Phase 17-18: True Streaming Shared Layer Backprop"
                ...
                Input_i[Input Chunk 'i']:::data & SampleMask --> K17["<b>(17) backprop_shared_weights...</b>"]:::kernel; K17 --> PARTIALS_Grad_SW_i["PARTIAL Grad_SW 'i'"]:::partial_data
                hidden_i --> K17 & K18; Summed_Grad_H -- slice --> K17 & K18
                SampleMask --> K18["<b>(18) backprop_shared_biases...</b>"]:::kernel; K18 --> PARTIALS_Grad_SB_i["PARTIAL Grad_SB 'i'"]:::partial_data
            end

            subgraph "Phase 19: Recursive Reduction (Shared Grads)"
                 K_Recursive_19["<b>(19) Recursive Reduction...</b>"]:::host_logic
                 PARTIALS_Grad_SW_i --> K_Recursive_19; PARTIALS_Grad_SB_i --> K_Recursive_19
...
- **(20) `normalize_gradients`**: **[Utility Kernel]** Generic, element-wise scaling kernel.
  - **Contract:** Performs an element-wise division of an input buffer by a scalar value (`N`, the number of items in the batch). It consumes a `Summed_*` buffer and produces a **`Final_*`**, normalized gradient buffer, which is essential for numerically stable mini-batch training.

---

#### **Final Update & Retrieval Kernels**

- **(21) `D2H Async Copy`**: Re-numbered. A non-blocking Device-to-Host transfer of `Final Probs`, signaling completion via `inference_event`.
- **(22) `adam_update`**: Re-numbered. Generic optimizer kernel, invoked once per complete parameter group **per training batch**, after the **Batch Synchronization Point**.
...
- **(23) `clamp_temperatures`**: Re-numbered. Final utility kernel constraining temperature parameters.
- **(24) `final_batch_event`**: This logical node represents the final device-side event that signals the completion of all computations for the training batch, including all parameter updates.
...
- **Scenario: The Hydra (Massive `num_heads`)**
  ...
  - **Insight:** Validates the intelligence of the **Reduction Planner**. For a massive number of heads, the host renders an efficient `log_K(N)` reduction tree, not just for diagnostics like loss, but for the **core `Summed_Grad_ModW` accumulation**, proving the engine is the heart of the learning algorithm itself.
