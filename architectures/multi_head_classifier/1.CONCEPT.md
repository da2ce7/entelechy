...
graph TD
    ...
    classDef unused_utility_kernel fill:#eceff1,stroke:#90a4ae,stroke-width:1px,stroke-dasharray: 2 2;
    ...
    %% === SETUP PHASE ===
    subgraph Phase0["Phase 0: Host Setup & Global Params"]
        HL_0["Start Batch"]:::host_logic --> HL_1["VRAM Budgeting & Chunking"]:::host_logic
        HL_1 --> HL_2["Read Mode, Manage Activations,<br/>& Plan Reductions"]:::host_logic
        ...
        P_ClassifierModule["Classifier Module Params"]:::param
    ...
    %% === ACT PHASE ===
    subgraph Phase1["Phase 1: Act (Forward Pass & Loss)"]
        K4["(4) forward_pass"]:::kernel
        HL_1 & SampleMask --> K4
        K4 --> hidden_i["Hidden Activations<br/>Chunk 'i' [C,R]"]:::data
        K4 --> hidden_mask["Hidden Mask<br/>Chunk 'i' [C,R]"]:::data
        K5["(5) render_logits_chunk"]:::kernel
        hidden_i & hidden_mask --> K5
        K5 --> Full_Logits["Full Logits Buffer [C,R]"]:::full_intermediate
        HL_PathSelect["Host Selects Loss Path..."]:::host_logic
    ...
    %% === LEARN PHASE ===
    subgraph Phase2["Phase 2: Learn (Gradient Computation & Reduction)"]
        K8_Start["K8 Start Trigger"]:::host_logic

        subgraph Phase2A["Phase 2A: Initial Gradient Production"]
            style Phase2A loop_box
            note_grad_loop["Executes in parallel for each of N items"]:::logical_step
            K8_Start --> note_grad_loop
            K8["(8) calculate_module_param_grads"]:::kernel
            note_grad_loop & PARTIALS_Probs & Targets & SampleMask & hidden_i --> K8
            ...
            K9["(9) backprop_error_to_hidden_chunk"]:::kernel
            note_grad_loop & PARTIALS_Probs & Targets & P_ClassifierModule & SampleMask --> K9
            K9 --> PARTIALS_Grad_H_AoS["Collection: PARTIAL Grad_H (AoS)"]:::partial_data
            K10["(10) calculate_chunk_temp_gradients"]:::kernel
            note_grad_loop & PARTIALS_Probs & Full_Logits & Targets & P_Temps & SampleMask --> K10
            K10 --> PARTIALS_Grad_Temps["Collection: PARTIAL Grad_Temps"]:::partial_data
        end

        subgraph Phase2B["Phase 2B: Gradient Processing & Permutation"]
            ...
            Clipped_PARTIALS_Grad_H_AoS --> K13
            K13 --> Permuted_Grad_H_SoA["Permuted Grad_H (SoA)"]:::full_intermediate
        end

        subgraph Phase2C["Phase 2C: Recursive Clip-Aggregation Engine"]
            style Phase2C engine_box
            K_Recursive_14["(14) Reduction (Act Results)"]:::host_logic
            Offset_List & PARTIALS_Probs & PARTIALS_Loss_BCE --> K_Recursive_14
            K_Recursive_14 --> FINAL_Probs["Final Probs"]:::final_data
            K_Recursive_14 --> FINAL_BCE_Loss["Final BCE Loss"]:::final_data

            subgraph PathA["Engine Path A: Module & Temp Gradients (Node 15)"]
                style PathA logical_step
                ...
                Host_Threshold_Calc1["Host calculates T_j"]:::host_logic
                P_ClippingPolicy --> Host_Threshold_Calc1
                K15b["(15b) clip_stage_j"]:::utility_kernel
                Host_Threshold_Calc1 & K15_Intermediate --> K15b
                ...
                K15_Recursive_Note --> Summed_Grad_ModW["Summed Grad_ModW"]:::summed_data
                K15_Recursive_Note --> Summed_Grad_ModB["Summed Grad_ModB"]:::summed_data
                K15_Recursive_Note --> Summed_Grad_Temps["Summed Grad_Temps"]:::summed_data
            end

            K16["(16) stabilize_and_reduce_grad_hidden_activations <br/><i>Implements log_K(M) reduction internally</i>"]:::specialized_kernel
            Permuted_Grad_H_SoA & P_ClippingPolicy --> K16
            K16 --> Summed_Grad_H["Summed Grad_H"]:::summed_data

            subgraph PathB["Engine Path B: Shared Gradients (Node 20)"]
                style PathB logical_step
                K20a["(20a) aggregate_stage_j"]:::kernel
                Clipped_PARTIALS_Grad_SW_i_Collection["Collection: Clipped PARTIAL Grad_SW 'i'"]:::clipped_data
                Clipped_PARTIALS_Grad_SB_i_Collection["Collection: Clipped PARTIAL Grad_SB 'i'"]:::clipped_data
                Clipped_PARTIALS_Grad_SW_i_Collection & Clipped_PARTIALS_Grad_SB_i_Collection & Offset_List --> K20a
                K20a --> K20_Intermediate["Intermediate<br/>Sum (Layer j)"]:::partial_data
                Host_Threshold_Calc2["Host calculates T_j"]:::host_logic
                P_ClippingPolicy --> Host_Threshold_Calc2
                K20b["(20b) clip_stage_j"]:::utility_kernel
                Host_Threshold_Calc2 & K20_Intermediate --> K20b
                K20b --> K20_Clipped_Intermediate["Clipped<br/>Intermediate (Layer j)"]:::clipped_data
                K20_Recursive_Note_B["...Recursion continues for<br>log_K(N)-1 stages..."]:::logical_step
                K20_Clipped_Intermediate --> K20_Recursive_Note_B
                K20_Recursive_Note_B --> Summed_Grad_SW["Summed Grad_SW"]:::summed_data
                K20_Recursive_Note_B --> Summed_Grad_SB["Summed Grad_SB"]:::summed_data
            end
        end

        subgraph Phase2D["Phase 2D: Shared Backpropagation"]
            note_streaming_model["<b>Host Strategy 2 Starts Here</b><br/>(Depends on Summed_Grad_H)"]:::host_logic
            Summed_Grad_H --> note_streaming_model

            subgraph BackpropLoop["Streaming Backprop Loop (Chunk 'i')"]
                style BackpropLoop parallel_group
                Input_i["Input Chunk 'i'"]:::data
                note_streaming_model --> Input_i
                K17["(17) backprop_shared_weights"]:::kernel
                Input_i & SampleMask & hidden_i --> K17
                Summed_Grad_H -. "slice" .-> K17
                K18["(18) backprop_shared_biases"]:::kernel
                SampleMask & hidden_i --> K18
                Summed_Grad_H -. "slice" .-> K18
                K17 --> PARTIALS_Grad_SW_i["PARTIAL Grad_SW 'i'"]:::partial_data
                K18 --> PARTIALS_Grad_SB_i["PARTIAL Grad_SB 'i'"]:::partial_data
                K19["(19) clip_shared_gradients_chunk<br><b>[Leaf-Level Clip: T_j=max]</b>"]:::utility_kernel
                PARTIALS_Grad_SW_i & PARTIALS_Grad_SB_i --> K19
                K19 --> Clipped_PARTIALS_Grad_SW_i_Collection
                K19 --> Clipped_PARTIALS_Grad_SB_i_Collection
            end
        end
    end

    %% === UPDATE PHASE ===
    subgraph Phase3["Phase 3: Update & Finalization"]
        ...
        subgraph Update["Final Update Stage"]
            ...
            K22_Barrier & FINAL_Grad_SW & FINAL_Grad_SB & P_Adam_Host --> K24_shared
            K24_shared -- "parameter updates" --> P_Shared
            K24_module["(24) adam_update (Module)"]:::kernel
            K22_Barrier & FINAL_Grad_ModW & FINAL_Grad_ModB & P_Adam_Host --> K24_module
            K24_module -- "parameter updates" --> P_ClassifierModule
            K24_temps["(24) adam_update (Temps)"]:::kernel
            K22_Barrier & FINAL_Grad_Temps & P_Adam_Host --> K24_temps
            K24_temps -- "parameter updates" --> P_Temps
            K25["(25) clamp_temperatures"]:::kernel
            K24_temps --> K25
            K25 --> EV_Final["final_batch_event"]:::sync_event
        end

        subgraph AsyncCopy["Async Result Retrieval"]
            K23["(23) D2H Async Copy (Final Probs)"]:::data
            FINAL_Probs --> K23
            K23 --> EV_Inference["inference_event"]:::sync_event
            Host_Act["Host Acts on<br/>Full Forward Result"]:::host_logic
            EV_Inference --> Host_Act
        end
    end
    EV_Final --> Host_Wait_Final["Host Blocks for<br/>Full Batch Completion"]:::host_logic

    %% INTER-PHASE CONNECTIONS
    Phase1 -. "Host trigger<br/>(e.g. label ready)" .-> K8_Start
