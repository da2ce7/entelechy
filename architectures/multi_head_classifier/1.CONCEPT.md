## **Architectural Concept: A Unified, Memory-Aware Streaming Classification Engine (Revision 4)**
    ...
    subgraph "PHASE 2: LEARN"
        ...
        subgraph "Phase 12-19: Gradient Processing & Reduction"
            ...
            subgraph "Phase 14: Recursive Reduction (Act Results)"
                 ...
                 PARTIALS_Probs & PARTIALS_Loss_BCE --> K_Recursive_14; K_Recursive_14 --> FINAL_Probs[Final Probs]:::final_data & FINAL_BCE_Loss[Final BCE Loss]:::final_data
            end

            subgraph "Phase 15: Recursive Reduction (Module Grads)"
                 K_Recursive_15["<b>(15) Recursive Reduction...</b>"]:::host_logic
                 Clipped_PARTIALS_Grad_ModW & Clipped_PARTIALS_Grad_ModB & Clipped_PARTIALS_Grad_Temps --> K_Recursive_15
            ...
            subgraph "Phase 19: Recursive Reduction (Shared Grads)"
                 ...
                 Clipped_PARTIALS_Grad_SW_i --> K_Recursive_19; Clipped_PARTIALS_Grad_SB_i --> K_Recursive_19
...
- **(14), (15), (19) `aggregate_*` kernels**: A suite of stateless reduction kernels serving as the **Recursive, Tiered Aggregation Engine**.
  - **Contract:** This engine is composed by the host to aggregate partial results. It sums Partial Probabilities (`Node 14`), **Clipped** Partial Module Gradients (`Node 15`), and **Clipped** Partial Shared Gradients (`Node 19`). The output of a gradient aggregation is a **`Summed_*`** gradient buffer.
...
- **Scenario: The Data Tsunami (Massive `batch_size`)**
  ...
  - **Insight:** This is the canonical test of the full parallel learning model. It validates that the host correctly processes a large training batch (`N` items) by:
    ...
    3.  Gathering the **clipped** partial gradient results via the Placement Contract.
    4.  Planning and executing the `log_K(N)` reduction tree to sum the **clipped** partial gradients.
    5.  Correctly inserting the **`(20) normalize_gradients`** step.
    6.  Enforcing the strict **`(21) Batch Synchronization Point`**, ensuring the `(23) adam_update` kernel only runs _after_ all preceding steps are complete.
