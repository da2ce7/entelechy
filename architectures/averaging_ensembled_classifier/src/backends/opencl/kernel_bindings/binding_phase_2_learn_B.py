...
class ClipPartialGradientsBinding(KernelBinding):
    ...
    def compute_grid(self, tile_index: int, scalar_params: dict[str, int | float], hardware_simd_width: int) -> tuple[tuple[int, ...], tuple[int, ...] | None]:
        ...
        padded_class_count = int(scalar_params["padded_class_count"])
    ...
    def marshal_args(self, get_buffer: Callable[[BufferHandle], cl.Buffer], buffer_bindings: dict[str, BufferHandle], scalar_params: dict[str, int | float], tile_index: int) -> list[Any]:
        ...
        return [
            ...
            get_buffer(buffer_bindings["grad_weights_module"]),
            get_buffer(buffer_bindings["grad_biases_module"]),
            get_buffer(buffer_bindings["grad_temps"]),
            get_buffer(buffer_bindings["grad_hidden_activations_aos"]),
            get_buffer(buffer_bindings.get(
                "clipping_threshold_per_item",
                buffer_bindings.get("null_buffer", buffer_bindings["grad_weights_module"]),
            )) if scalar_params.get("use_per_item_norm", 0) else None,
            get_buffer(buffer_bindings["clipped_grad_weights_module"]),
            get_buffer(buffer_bindings["clipped_grad_biases_module"]),
            get_buffer(buffer_bindings["clipped_grad_temps"]),
            get_buffer(buffer_bindings["clipped_grad_hidden_activations_aos"]),
            np.uint32(scalar_params.get("use_per_item_norm", 0)),
            np.float32(scalar_params.get("clipping_threshold_global", 0.0)),
            ...
            np.uint32(scalar_params["padded_class_count"]),
...
class GatherAndPermuteGradHBinding(KernelBinding):
    ...
    def marshal_args(self, get_buffer: Callable[[BufferHandle], cl.Buffer], buffer_bindings: dict[str, BufferHandle], scalar_params: dict[str, int | float], tile_index: int) -> list[Any]:
        return [
            get_buffer(buffer_bindings["clipped_partials_aos"]),
