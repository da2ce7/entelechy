# tests/tier2/opencl/test_learn_streaming_backprop.py
"""Tier 2 tests: streaming loop backprop (Nodes 17-19)."""
...
class TestStreamingBackprop:
    """Per-kernel correctness tests for streaming loop backprop."""
    ...
    def test_streaming_multi_chunk(self):
        ...
        # Verify all partials have correct shape
    ...
    def test_streaming_masked_samples(self):
        ...
        grad_h = rng.standard_normal((batch, hid)).astype(np.float32)
        x = make_input_data(rng, batch, inp)
        mask = np.zeros(batch, dtype=np.float32)  # All masked
