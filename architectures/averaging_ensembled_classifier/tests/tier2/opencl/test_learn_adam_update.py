# tests/tier2/opencl/test_learn_adam_update.py
"""Tier 2 tests: adam_update kernel (Node 24)."""
...
class TestAdamUpdate:
    """Per-kernel correctness tests for adam_update."""
    ...
    def test_adam_update_basic(self):
        ...
        t = 1
        beta1_pow_t = np.float64(beta1) ** t
        beta2_pow_t = np.float64(beta2) ** t
        ...
        np.testing.assert_allclose(ref_m1, beta1 * m1 + (1 - beta1) * grads,
    ...
    def test_adam_update_bias_correction(self):
        ...
        # m1_hat = m1_new / (1 - beta1^t) should amplify early gradients
    ...
    def test_adam_update_fp64_bias_terms(self):
        ...
        _beta1, beta2 = 0.9, 0.999
        # After 1000 steps, beta2^1000 in FP32 would underflow
        t = 1000
        beta2_pow_fp64 = np.float64(beta2) ** t
        _beta2_pow_fp32 = np.float32(beta2) ** t
        # FP64 should retain precision; FP32 may underflow to 0
    ...
    def test_adam_update_per_param_group(self):
        ...
        lr, beta1, beta2, eps = 0.001, 0.9, 0.999, 1e-7
        t = 1
        b1t = float(np.float64(beta1) ** t)
        b2t = float(np.float64(beta2) ** t)
        ...
        # Group B: biases (different shape)
