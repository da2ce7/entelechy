# tests/tier2/opencl/test_learn_clip_partials.py
"""Tier 2 tests: clip_partial_gradients kernel (Node 11)."""
...
class TestClipPartialGradients:
    """Per-kernel correctness tests for clip_partial_gradients."""
    ...
    def test_clip_near_threshold_epsilon_placement(self):
        ...
        Verifies scale = threshold / (norm + epsilon), matching the
        kernel contract in kernels.cl.h. Uses small threshold so the
        formulas diverge measurably.
        """
        epsilon = 1e-7
        threshold = 1e-3
