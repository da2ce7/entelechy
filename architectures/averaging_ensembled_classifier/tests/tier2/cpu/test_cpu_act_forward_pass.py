# tests/tier2/cpu/test_cpu_act_forward_pass.py
"""Tier 2 CPU tests: forward_pass kernel (Node 4)."""
...
class TestCpuForwardPass:
    """Per-kernel correctness tests for CPU forward_pass."""
    ...
    def test_forward_pass_batch_sizes(self, batch_size: int):
        """Correctness across different batch sizes, including non-power-of-2."""
        ...
        assert ref_mask.shape == (batch_size, hid)
        assert not np.any(np.isnan(ref_act))

    def test_forward_pass_zero_input(self):
        """Forward pass with all-zero input produces bias-only activations."""
        rng = make_rng(seed=46)
        batch, inp, hid = 4, 4, 8
        x = np.zeros((batch, inp), dtype=np.float32)
        w = make_weights(rng, hid, inp)
        b = make_biases(rng, hid)
        mask = np.ones(batch, dtype=np.float32)

        ref_act, _ = ref_forward_pass(x, w, b, mask)
        tol = get_tolerance("forward_pass", "fp32")

        # With zero input, pre_act = bias only
