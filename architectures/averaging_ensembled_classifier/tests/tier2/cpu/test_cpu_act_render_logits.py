# tests/tier2/cpu/test_cpu_act_render_logits.py
"""Tier 2 CPU tests: render_logits_chunk kernel (Node 5)."""
...
class TestCpuRenderLogits:
    """Per-kernel correctness tests for CPU render_logits_chunk."""
    ...
    def test_render_logits_single_class(self):
        """Edge case: single output class."""
        rng = make_rng(seed=53)
        batch, hid, cls = 4, 8, 1
        hidden_act = np.abs(make_input_data(rng, batch, hid))
        w = make_weights(rng, cls, hid)
        b = make_biases(rng, cls)

        ref = ref_render_logits_chunk(hidden_act, w, b, temperature=1.0)

        assert ref.shape == (batch, 1)
        assert not np.any(np.isnan(ref))
