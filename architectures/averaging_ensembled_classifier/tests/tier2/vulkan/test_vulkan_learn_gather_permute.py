# tests/tier2/vulkan/test_vulkan_learn_gather_permute.py
"""Tier 2 Vulkan tests: gather_and_permute_grad_h kernel (Node 13)."""
...
class TestVulkanGatherPermute:
    """Per-kernel correctness tests for Vulkan gather_and_permute_grad_h."""
    ...
    def test_gather_permute_identity(self):
        """Single tile: identity permutation."""
        rng = make_rng(seed=132)
        tile = rng.standard_normal((8, 16)).astype(np.float32)
        ref = ref_gather_and_permute_grad_h([tile])
        np.testing.assert_array_equal(ref, tile)

    def test_gather_permute_preserves_values(self):
        """All input values appear in output."""
        rng = make_rng(seed=133)
        tiles = [rng.standard_normal((4, 4)).astype(np.float32) for _ in range(4)]
        ref = ref_gather_and_permute_grad_h(tiles)
        all_input = np.concatenate(tiles, axis=0)
        np.testing.assert_array_equal(np.sort(ref.ravel()), np.sort(all_input.ravel()))
