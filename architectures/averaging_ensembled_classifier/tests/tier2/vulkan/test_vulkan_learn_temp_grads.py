# tests/tier2/vulkan/test_vulkan_learn_temp_grads.py
"""Tier 2 Vulkan tests: calculate_chunk_temp_gradients kernel (Node 10)."""
...
class TestVulkanTempGrads:
    """Per-kernel correctness tests for Vulkan temperature gradients."""
    ...
    def test_temp_grads_zero_gradient(self):
        """When probs == targets, temperature gradient approaches zero."""
        batch, cls = 4, 3
        targets = np.zeros((batch, cls), dtype=np.float32)
        targets[:, 0] = 1.0
        probs = targets.copy()
        logits_unscaled = np.zeros((batch, cls), dtype=np.float32)
        temperature = 1.0
        mask = np.ones(batch, dtype=np.float32)

        grad_t = ref_temp_gradients_cce(probs, targets, logits_unscaled, temperature, mask)

        np.testing.assert_allclose(grad_t, 0.0, atol=1e-6)

    def test_temp_grads_masked(self):
        """Fully masked batch produces zero temperature gradient."""
