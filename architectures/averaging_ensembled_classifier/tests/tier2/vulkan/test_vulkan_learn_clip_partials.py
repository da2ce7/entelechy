# tests/tier2/vulkan/test_vulkan_learn_clip_partials.py
"""Tier 2 Vulkan tests: clip_partial_gradients kernel (Node 11)."""
...
class TestVulkanClipPartialGradients:
    """Per-kernel correctness tests for Vulkan clip_partial_gradients."""
    ...
    def test_clip_near_threshold_epsilon_placement(self):
        ...
        NOT threshold / sqrt(norm^2 + epsilon). At small norms where epsilon
        is comparable to norm^2, the formulas diverge measurably.
        """
        epsilon = 1e-7
        threshold = 1e-3
        norm_target = threshold + 1e-5
        ...
        assert not np.allclose(ref, wrong_result, atol=1e-5), (
            "Clipped result should differ from sqrt(norm^2 + eps) formula"
        )
