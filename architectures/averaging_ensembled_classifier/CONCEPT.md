## **Architectural Concept: A Unified, Memory-Aware Streaming Classification Engine (Revision 10)**

### **Guiding Principles**

#### 1. **Architectural Elegance Feedback**

Optimization pressure that violates these core principles shall be interpreted as useful signal that indicates incomplete architectural modeling, thus is no justification for exceptions. The system must evolve its formal abstractions to subsume valid optimizations as first-class primitives, never compromise its contracts.
...
- **1. Conceptual (This Document):** Sovereign authority defining **what** must be achieved and **why**. Contains all principles, component definitions, and validation scenarios. Structural decisions that realize these mandates are recorded in the ADR chain; once accepted, an ADR's decision is binding on all downstream layers.
...
At the core of the architecture lies a powerful aggregation engine that implements a **recursive, multi-stage reduction tree**. Governed by a host-resolved parameter, `K`—the **Reduction Batch Size**—this engine defines the width of the parallel kernel front at each reduction stage. The realized value of `K` is the resolution of two independent constraints:

**`K = min(K_hw, K_policy)`**

- **`K_hw`** is sourced from `HardwareProfile.max_reduce_fan_in` — the maximum fan-in the target device can efficiently sustain per reduction stage.
- **`K_policy`** is a user-configurable hyperparameter governing stabilization granularity — the fan-in the policy *desires*. Defaults to `K_hw` when unspecified, making the hardware ceiling binding and preserving current default behavior.

**Terminological Note.** The symbol `K` in this document always denotes the Reduction Batch Size as resolved above. At a specific stage `j`, the actual fan-in `K_j` equals `K` for all non-tail nodes and equals the remainder (or fewer) for the tail node; absent partials in the tail node are represented by a sentinel value. The symbol `K` is never used for kernel-internal quantities such as workgroup tree branching factors or per-thread accumulation counts, which are described by their own named symbols where they appear.

This `log_K(N)` strategy breaks the `O(N)` memory dependency of a flat aggregation model, enabling scalability to problems of arbitrary size. The engine employs three tiers of aggregation (Identity, Register, Local) selected by the Host Orchestrator based on the number of partials being reduced at any given stage. The **Identity** tier is an Orchestration-tier bypass: when `N=1`, no reduction kernel is dispatched and the single partial is referenced directly as the destination buffer. The **Register** (`aggregate_register_reduce`) and **Local** (`aggregate_local_reduce`) tiers are Execution-tier kernels selected by a hardware-informed crossover heuristic based on reduction width.
...
**Execution-Tier Internal Amplification.** GPU backends (OpenCL, Vulkan) implement Node 16 using a "work-group per row" strategy where each thread first accumulates `L_pre = ⌈total_modules_count / workgroup_size⌉` elements (the **pre-accumulation count**) before the staged reduction begins. The `P = min(total_modules_count, workgroup_size)` post-accumulation intermediates then enter the staged tree reduction. The Orchestration tier accounts for this pre-accumulation when rendering Node 16's threshold schedule: it computes a pre-accumulation threshold and a `⌈log_K(P)⌉`-stage schedule over the `P` post-accumulation intermediates. The kernel applies these host-prescribed thresholds without independent policy computation, consistent with the threshold injection pattern used by the generic reduction engine (Nodes 14/15/20).

The pre-accumulation threshold is derived from the first clip stage's overflow constraint — not from the pre-accumulation count itself:

**`T_pre = COMPUTE_FP_FORMAT_MAX / K`**

This bound ensures that when the first clip stage sums `K` post-accumulation values (each bounded by `T_pre`), the result cannot exceed `COMPUTE_FP_FORMAT_MAX`. The quantity `L_pre` does not appear in this formula. Mid-accumulation overflow (during the serial summation of `L_pre` elements per thread) is prevented by the upstream constraint on Node 11's threshold — specifically, the Pre-Summation Amplification invariant `T₁₁ × num_class_chunks ≤ COMPUTE_FP_FORMAT_MAX` ensures each input element is bounded well below `COMPUTE_FP_FORMAT_MAX / L_pre`.
...
3. **Backward propagation:** Node 8's `ZERO_REQUIRED` output initialization ensures class-dimension and hidden-dimension padding carries zero (host is the guarantor; kernel applies Padding Zero-Preservation — each dispatch writes only within its assigned class chunk range; unwritten positions including class-dimension padding retain the host-initialized zero; hidden-dimension padding produces zero gradients because upstream hidden activations at padding indices are zero). When `num_batch_chunks > 1`, the batch-chunk ReductionTreeNode sums zeros (arithmetic consequence), and the Precision Bridge (`narrow_to_storage`) preserves zeros (format conversion identity: `store_storage(0) == 0` for all supported formats including FP8). Node 11 applies Padding Zero-Preservation via uniform scaling (zero × scale = zero for all finite scaling factors). Node 13's `ZERO_REQUIRED` output initialization ensures padded module positions carry zero (host is the guarantor; kernel applies Padding Zero-Preservation). **Node 16** reduces across modules, propagating zeros at padded hidden positions into `summed_grad_h` (Padding Zero Propagation (Emergent): sum of zeros across the module reduction dimension is zero). Nodes 17 and 18 apply Padding Zero-Establishment for their output buffers (which have `Initialization Contract: NOT_REQUIRED`), explicitly writing zeros to padding positions via their `input_count`/`hidden_count` boundary parameters. Node 19 applies Padding Zero-Preservation via uniform scaling. Node 20's reduction sums zeros (arithmetic consequence). **Node 21** divides zeros by the batch size, propagating zeros (Padding Zero Propagation (Emergent): `0 / divisor == 0` for all finite positive divisors). Node 24 processes zero gradients with zero moments, producing zero parameter updates (Padding Zero-Preservation (Inductive)).
...
- **Orchestration Tier (Per-Backend Rendering):** Receives the execution plan and renders it using the backend's native dispatch model. OpenCL enqueues per-tile `clEnqueueNDRange` calls with event chains. Vulkan records a command buffer with `vkCmdDispatch` and `vkCmdPipelineBarrier`. CPU calls `pool_dispatch_and_wait` per node. Kernel tier selection for reduction trees (register-reduce vs. local-reduce), buffer allocation via native allocators, and synchronization primitive management are Orchestration-tier concerns.
  ...
  **Inter-Kernel Precision Boundary Resolution.** When a DAG edge connects a compute-role producing buffer to a consumer kernel expecting a storage-role input (or vice versa), and neither variant selection (ADR-026) nor the consumer's native mixed-role interface resolves the mismatch, the Orchestration tier SHALL insert a standalone Precision Bridge dispatch (`narrow_to_storage` or, if formalized in future, `widen_to_compute`) at the DAG edge. The bridge is a `KernelDispatchNode` conditioned on `PrecisionConfig.storage_dtype != PrecisionConfig.compute_dtype` — when storage and compute types are identical, the bridge is elided and the source buffer is passed directly, because the C types are bit-compatible. This rule is a Policy-tier concern: the plan-construction logic inspects the `precision_role` of the source `BufferDescriptor` and the consuming kernel's `KernelContract`, and inserts the bridge when roles diverge.
...
4.  **Reduction Planning & Rendering:** Analyzes problem size and device capabilities (via `HardwareProfile`) to select an optimal Reduction Batch Size (`K`), rendering the full `log_K(N)` reduction tree as `ReductionTreeNode`s with pre-computed threshold schedules and initial offset lists. For Node 16, the Orchestration tier additionally renders a per-stage threshold schedule and pre-accumulation threshold, adapted to the backend's dispatch topology. In both cases, `K` is the Reduction Batch Size — the same value used for the generic reduction engine's `ReductionTreeNode` planning. The CPU backend renders the `⌈log_K(M)⌉`-stage schedule directly (no pre-accumulation; `M = total_modules_count`). GPU backends first account for pre-accumulation: each of `W` threads (workgroup size) accumulates `L_pre = ⌈M/W⌉` elements, yielding `P = min(M, W)` post-accumulation intermediates; the Orchestration tier then renders a `⌈log_K(P)⌉`-stage schedule with a pre-accumulation threshold of `COMPUTE_FP_FORMAT_MAX / K` (see §3.4 for derivation). This uniformity of `K` across all reduction paths is a Policy-tier choice, not an interface constraint — the kernel receives `num_reduction_stages` and `clipping_threshold_per_stage` without knowledge of `K`. The rendered schedule is passed to the kernel as a buffer parameter, paralleling the per-stage threshold scalars passed to the generic `reduce_k_fan_in_and_clip` kernels.
...
6.  **Partial Result Placement & Indexing:** Provides each tile-parallel kernel invocation with a unique placement key via abstract placement strategies (e.g., `grid_mod_cls`, `grid_mod_cls_batch`, `linear_batch`), guaranteeing non-overlapping writes. The mechanism by which each tile resolves its index—host scalar, hardware intrinsic, or task parameter—is a `KernelBinding` concern.
7.  **Phase Staging & Optimizer State Management:** Governs phase intervals through named synchronization points (`BarrierNode`s and `RetrievalNode`s). For the Adam optimizer, the host computes `beta1**t` and `beta2**t` bias correction terms in high precision (FP64) and passes the final values to the update kernel, avoiding on-device precision loss. Precision is configured through a `PrecisionConfig` frozen dataclass carrying three independent role assignments: `storage_dtype` (bandwidth optimization), `compute_dtype` (arithmetic fidelity), and `state_dtype` (optimizer stability), along with five derived scalar constants: `storage_fp_format_max`, `storage_fp_min_positive`, `storage_mantissa_bits`, `compute_fp_format_max`, and `compute_epsilon`. A configuration where all three roles share a type (e.g., `PrecisionConfig.float32()`) is a parameterization — not a distinct mode. The host's existing practice of computing Adam bias correction terms in FP64 is recognized as an instance of state-precision independence; the three-role model formalizes this pattern as a first-class concern rather than an ad-hoc exception.
...
All kernels designated as "Partial Renderers" must accept a unique placement key from the host. This key is used to calculate the write offset within their designated output buffer, ensuring each partial result is placed in its correct, discrete slot. Kernels using the `grid_mod_cls` strategy receive a `flat_tile_index`; those using `grid_mod_cls_batch` receive both a `flat_tile_index` and a `batch_chunk_index`. This contract applies to kernels: **(6), (7), (8), (9), (10), (11)**.
...
- **(4) `forward_pass`**: Computes hidden activations **for a specified data chunk**.
  - **Contract:** Applies a (Weights \* Input + Bias + ReLU) transform, producing an `hidden_i` buffer for a single data chunk. Supports both Cache and Recompute strategies. Hidden mask production is controlled by a `FLAG` scalar: when set, the ReLU derivative mask capturing compute-precision truth is written before activation storage narrowing; when cleared, mask writes are skipped.
- **(5) `render_logits_chunk`**: A streamable kernel computing raw logits from a **data chunk of hidden activations**.
  - **Contract:** Executes a (Weights \* Input + Bias) transform, yielding `Full_Logits` that support caching and recomputation. Employs a sparsity-aware dot product that elides terms corresponding to ReLU-zeroed hidden units. The ReLU derivative source is selected by a `FLAG` scalar (explicit mask buffer vs. recomputed from stored activations).
...
- **(8) `calculate_module_param_grads_chunk`**: Computes partial gradients for module parameters from a **data chunk of hidden activations**.
  - **Contract:** Utilizes `Partial_Probs`, `Targets`, and a **chunk** of `hidden_i` to generate partial `Grad_ModW` and `Grad_ModB`. Adheres to the Placement Contract via the `grid_mod_cls_batch` strategy, placing each (tile, batch_chunk) pair in a unique slot. When `num_batch_chunks > 1`, a `ReductionTreeNode` sums across batch chunks, producing compute-role output; the Orchestration tier inserts a `narrow_to_storage` Precision Bridge when `storage_dtype != compute_dtype` to restore storage-role compatibility before Node 11 computes the joint L2 norm. When `storage_dtype == compute_dtype`, the bridge is elided. When `num_batch_chunks == 1`, the Identity tier bypass applies.
- **(9) `backprop_error_to_hidden_chunk`**: Computes the partial upstream gradient (`Grad_H`) for a tile of the problem.
  - **Contract:** Backpropagates errors using `Partial_Probs` and `Targets`. Adheres to the Placement Contract via the `grid_mod_cls` strategy. This kernel processes the **complete batch dimension** in a single dispatch — batch-chunk decomposition is intentionally absent because the downstream Item Synchronization Point (Node 13) requires a monolithic collection buffer.
...
- **(13) `gather_and_permute_grad_h`**: **[Specialized Kernel]** Gathers and permutes the clipped `Grad_H` partials.
  - **Contract:** Reads from the `Clipped_PARTIALS_Grad_H_AoS` collection and writes to a single `Permuted_Grad_H_SoA` buffer, performing an implicit summation over the `class_chunk` dimension during the gather. This is a mandatory step before Node (16) and serves as the canonical **Item Synchronization Point**.
...
- **`narrow_to_storage`**: **[Utility Kernel]** Element-wise precision bridge from compute role to storage role.
  - **Contract:** Pure format conversion: reads COMPUTE_TYPE elements directly, narrows each to STORAGE_TYPE via `store_storage()`. No arithmetic is performed on the values. Inserted by the Orchestration tier at DAG edges where a compute-role producer feeds a storage-role consumer and no intermediate kernel naturally performs the conversion (see §5, Inter-Kernel Precision Boundary Resolution). When `storage_dtype == compute_dtype`, the Orchestration tier elides this dispatch entirely — the source buffer is bit-compatible with the consumer's expectation.
  - **Padding Invariant:** Declares `Padding Zero Propagation (Emergent)`: zero-valued positions in the input map to zero-valued positions in the output because `store_storage(0) == 0` for all supported precision formats, including FP8 where `0x00` represents zero in both E4M3 and E5M2.
  - **Invocation:** Currently used between the batch-chunk `ReductionTreeNode` (compute-role output) and Node 11 (storage-role input) when `num_batch_chunks > 1` and `storage_dtype != compute_dtype`.
...
- **(16) `stabilize_and_reduce_grad_hidden_activations`**: **[Specialized Kernel]** A parametric reduction engine that applies a host-prescribed stabilization schedule to the `Grad_H` vector.
  ...
  - **Padding Invariant:** Declares `Padding Zero Propagation (Emergent)`: hidden-dimension padding positions (indices `h ≥ hidden_count` within `padded_hidden_count`) in the output buffer carry zero when the input buffer has zero at the corresponding module-column positions for all modules. This is a mathematical consequence of summing zeros across the module reduction dimension. Precondition: Node 13's Padding Zero-Preservation ensures zero at padding positions in the input SoA buffer.
...
- **(17) `backprop_shared_weights_chunk`**: Computes partial gradients for shared weights.
  - **Contract:** Consumes `Input_i`, a slice of the `Summed_Grad_H`, and a **chunk** of recomputed `hidden_i`. Writes to a single-slot scratch buffer consumed by Node (19) within the same streaming loop iteration. The ReLU derivative source is selected by a `FLAG` scalar (explicit mask buffer vs. recomputed from stored activations). Output uses the SIMD-major (SoA) layout matching the persistent shared weight parameter layout.
...
- **(18) `backprop_shared_biases_chunk`**: Computes partial gradients for shared biases.
  - **Contract:** Consumes a slice of the `Summed_Grad_H` and a **chunk** of recomputed `hidden_i`. Writes to a single-slot scratch buffer consumed by Node (19) within the same streaming loop iteration. The ReLU derivative source is selected by a `FLAG` scalar (explicit mask buffer vs. recomputed from stored activations).
...
- **(21) `normalize_gradients`**: **[Utility Kernel]** Generic, element-wise scaling kernel.
  - **Contract:** Performs an element-wise division of an input buffer by a scalar value (`effective_batch_size`, the number of valid samples in the batch as computed by host-side popcount over the sample mask). It consumes all `Summed_*` parameter gradient buffers (`Summed_Grad_ModW`, `Summed_Grad_ModB`, `Summed_Grad_Temps`, `Summed_Grad_SW`, `Summed_Grad_SB`) and produces their corresponding **`Final_*`**, normalized gradient buffers, which are essential for numerically stable mini-batch training.
  - **Padding Invariant:** Declares `Padding Zero Propagation (Emergent)`: positions in the output buffer corresponding to padding indices carry zero when the input buffer has zero at those positions, because `output[i] = input[i] / divisor` maps zero to zero for all finite positive divisors. Precondition: the input buffer's zeros at padding positions are guaranteed by the upstream reduction chain (Nodes 11, 15/20) which preserves the zero invariant established by Nodes 8, 9, 17, and 18.
...
- **(24) `adam_update`**: Generic optimizer kernel, invoked once per complete parameter group **per training batch**, after the **Batch Synchronization Point (22)**.
  - **Contract:** Applies the Adam update to a parameter buffer slice addressed by `(parameter_offset, parameter_count)` within a total buffer of `total_parameter_count` elements. It is a stateless numerical function that receives a **`Final_Grad_*`** buffer (the **fully reduced and normalized batch-wide gradient**), its corresponding moment vectors (`m1`, `m2`), and the host-computed hyperparameters. The slice-access pattern (ADR-030) enables per-module-chunk invocation when the module dimension is decomposed.
- **(25) `clamp_temperatures`**: Final utility kernel constraining temperature parameters within a [min, max] range, operating on a slice addressed by `(parameter_offset, parameter_count)`.
