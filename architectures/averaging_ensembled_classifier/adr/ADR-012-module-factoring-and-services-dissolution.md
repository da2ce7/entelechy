...
| `phase_2_learn_C_reduction.py` | Aggregation kernels (`aggregate_register_reduce`, `aggregate_local_reduce`), `clip_intermediate_grad` | — | Used within `ReductionTreePlan` rendering. The Identity tier (N=1 bypass) is an Orchestration-tier concern, not a kernel contract. |
